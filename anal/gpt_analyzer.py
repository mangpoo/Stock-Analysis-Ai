...
{stock_data.get('stock_code')}의 가격은 {current_price:.2f}{currency}이에요. 변동폭은 {change_rate:+.2f}%이에요. 위 차트에서의 1차 지지선은 [지지선1]{currency}이고, 2차 지지선은 [지지선2]{currency}이에요. 1차 저항선은 [저항선1]{currency}이고, 2차 저항선은 [저항선2]{currency}이에요. [최근 추세 분석 내용]. 앞으로의 매매 전략을 신중히 고려해보세요.
"""
